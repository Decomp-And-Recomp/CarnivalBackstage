...
internal class BufferReader
{
    ...
    public bool CheckBytesLeft(int left)
    {
        ...
        {
            //Debug.LogWarning("m_data.Length=" + m_data.Length + "  m_offset=" + m_offset + "  left=" + left);
            return false;
        }
        return true;
    }
    ...
    public bool PopUInt16(ref ushort val)
    {
        ...
        if (m_little_ending)
        {
            val = (ushort)((m_data[m_offset + 1] << 8) | m_data[m_offset]);
        }
        else
        {
    ...
    public bool PopUInt32(ref uint val)
    {
        ...
        if (m_little_ending)
        {
            val = (uint)((m_data[m_offset + 3] << 24) | (m_data[m_offset + 2] << 16) | (m_data[m_offset + 1] << 8) | m_data[m_offset]);
        }
        else
        {
    ...
    public bool PopByteArray(ref byte[] val) => PopByteArray(ref val, val.Length);
